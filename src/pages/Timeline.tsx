...
import { formatDate, formatDateRange, cx } from '@lib/format'
...
export default function Timeline() {
  ...
  const yearGroups: YearGroup[] = useMemo(() => {
    ...
    for (const r of rows) {
      const yr = parseInt(r.visit.arrivalDate.split('-')[0], 10)
  ...
  const filteredRows = useMemo(() => {
    return rows.filter((r) => {
      ...
      if (selectedYear) return parseInt(r.visit.arrivalDate.split('-')[0], 10) === selectedYear
      return true
    })
